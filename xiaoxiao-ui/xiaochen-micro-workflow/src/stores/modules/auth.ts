...
import { listTreeByMicroName, listTree } from "@/api/system/menu";
...
export const useAuthStore = defineStore({
  ...
  actions: {
    ...
    async getAuthMenuList() {
      //const microName = import.meta.env.VITE_MICRO_NAME;
      const props = window.$wujie?.props; // {data: xxx, methods: xxx}
      if (props) {
        const { data } = await listTreeByMicroName(props.microName);
