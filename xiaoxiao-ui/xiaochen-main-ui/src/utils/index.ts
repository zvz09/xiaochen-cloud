...
import { MicroInfo } from "@/api/modules/micro";
...
export const HomeMenu: Menu.MenuOptions = {
  path: HOME_URL,
  name: "home",
...
export function getMicroRouter(microInfos: MicroInfo[]) {
  let microRouter: Menu.MenuOptions[] = [];
  microRouter.push(HomeMenu);
  microInfos.forEach(microInfo => {
    microRouter.push({
      path: microInfo.path,
      name: microInfo.name,
      meta: {
        microUrl: microInfo.url,
        microName: microInfo.name,
        redirect: microInfo.redirect,
        icon: microInfo.icon,
        title: microInfo.title,
        isLink: "",
        isHide: false,
        isFull: false,
        isAffix: false,
