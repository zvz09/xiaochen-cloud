...
import com.zvz09.xiaochen.k8s.manage.constant.YamlConstant;
...
public class K8sService {
    ...
    public String createToken(String kubeConfigStr) {
        ...
        try {
            ...
            createServiceAccount(coreV1Api,(V1ServiceAccount) Yaml.load(new StringReader(YamlConstant.SA_XIAOCHEN_ADMIN)));

            createClusterRoleBinding(rbacAuthorizationV1Api,(V1ClusterRoleBinding) Yaml.load(new StringReader(YamlConstant.CRB_XIAOCHEN_ADMIN)));

            V1Secret secret = createSecret(coreV1Api,(V1Secret) Yaml.load(new StringReader(YamlConstant.TOKEN_XIAOCHEN_ADMIN_TOKEN)));
    ...
    private static void createServiceAccount(CoreV1Api coreV1Api, V1ServiceAccount serviceAccount) {
        KubernetesResourceUtil.createResource(
                ...
                (apiClient, resource) -> coreV1Api.readNamespacedServiceAccountCall(Objects.requireNonNull(serviceAccount.getMetadata()).getName(), serviceAccount.getMetadata().getNamespace(), null, null),
                (apiClient, resource) -> coreV1Api.createNamespacedServiceAccountCall(Objects.requireNonNull(serviceAccount.getMetadata()).getNamespace(), resource, null, null, null, null, null),
                serviceAccount
        );
    }
    ...
    private void createClusterRoleBinding(RbacAuthorizationV1Api rbacAuthorizationV1Api, V1ClusterRoleBinding clusterRoleBinding) {
        KubernetesResourceUtil.createResource(
                ...
                (apiClient, resource) -> rbacAuthorizationV1Api.readClusterRoleBindingCall(Objects.requireNonNull(resource.getMetadata()).getName(), null, null),
                (apiClient, resource) -> rbacAuthorizationV1Api.createClusterRoleBindingCall(resource, null, null, null, null, null),
                clusterRoleBinding
        );
    }
    ...
    private V1Secret createSecret(CoreV1Api coreV1Api, V1Secret secret) {
        KubernetesResourceUtil.createResource(
                coreV1Api.getApiClient(),
                (apiClient, resource) -> coreV1Api.readNamespacedSecret(Objects.requireNonNull(resource.getMetadata()).getName(), Objects.requireNonNull(resource.getMetadata()).getNamespace(), null),
                (apiClient, resource) -> coreV1Api.createNamespacedSecret(Objects.requireNonNull(resource.getMetadata()).getNamespace(),resource, null, null, null, null),
                secret
        );
        try {
